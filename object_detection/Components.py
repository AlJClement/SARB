...
class Components():
    ...
    def _detect_objects(self):
        ...
        for pat_idx in range(len(self.img_torch)):
            ...
            #IF TOP LABEL IS bigger than 200000 rethreshold likley an artifact
            for j, label in enumerate(top_labels):
                if j == 0:
                    if stats.GetNumberOfPixels(label)>200000:
                        print('rethreshold')
                        name = self.img_details_arr[pat_idx][0]
                        ...
                        try:
                            pat_class = self.img_class_torch[pat_idx]
                        except:
                            pat_class = self.img_class_torch[0][pat_idx]
                        
                        if pat_class==1:    
                            upper_threshold=0.3 # Create binary mask: 1 where value is in [lower, upper], else 0
                        else:
                            upper_threshold=0.372                   # Create binary mask: 1 where value is in [lower, upper], else 0
                        binary = ((image_for_threshold >= self.lower_thresh) & (image_for_threshold <= upper_threshold )).astype(np.uint8)
